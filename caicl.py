...
from interaction_dist import ModifiedEncoder
...
class HumanObjectMatcher(nn.Module):
    def __init__(self, repr_size, num_verbs, obj_to_verb, dropout=.1, human_idx=0):
        ...
        # self.coop_layer1 = ModifiedEncoder(
        #     hidden_size=256,
        #     representation_size=128,
        #     num_layers=1,
        #     num_heads=8,
    ...
    def forward(self, region_props, image_sizes, device=None):
        ...
        for i, rp in enumerate(region_props):
            # every one
            boxes, scores, labels, embeds, hbxs, obxs = rp.values()
            # human_num
            ...
            # --------------------start 远近注意力
            scaled_boxes, areas = self.scale_box(boxes, image_sizes[i])
            pairwise_dist = self.get_pairwise_dist(boxes[x], boxes[y])
            pairwise_dist = pairwise_dist.reshape(n, n)
            embeds, unary_attn = self.coop_layer(embeds, pairwise_dist, scaled_boxes, x, y)
            # embeds, unary_attn = self.coop_layer1(embeds, pairwise_dist, scaled_boxes, x, y)
            # ------------------end 远近注意力
        ...
        # 人-物融合的特征，配对的序号，配对的得分，物体的序列号，位置编码特征
...
class CAICL(nn.Module):
    ...
    def __init__(self,
                 ...
                 triplet_decoder_mask:nn.Module,
                 # contact_decoder: nn.Module,
                 # non_contact_decoder:nn.Module,
        ...
        # self.non_contact_decoder = non_contact_decoder
        self.decoder_mask = triplet_decoder_mask
        ...
        self.contactalpha = contactalpha
        # self.contact_parameter = nn.Parameter(torch.tensor(0.2))
    ...
    @staticmethod
    def advanced_forward(ctx, samples: NestedTensor, contact_masks: List[Tensor]):
        if not isinstance(samples, NestedTensor):
            samples = nested_tensor_from_tensor_list(samples)
        features, pos = ctx.backbone(samples)

        # --------------------------- 统一大小
        batch_shape = samples.mask.shape
        tp = torch.zeros(batch_shape, device=samples.tensors.device)

        for i in range(len(contact_masks)):
            c_mask = contact_masks[i].squeeze(0)
            tp[i][: c_mask.shape[0], : c_mask.shape[1]].copy_(c_mask[:, :])
        # ---------------------------

        # c_mask = contact_masks.squeeze(0)
        # tp[0][: c_mask.shape[0], : c_mask.shape[1]].copy_(c_mask[:, :])
        ...
        for l, feat in enumerate(features):
            ...
            c_mask = F.interpolate(tp[None].float(), size=mask.shape[-2:])[0]
        ...
        return out, hs, features, c_masks

    def get_feature_ho_mask(self, feature_ho_bxs, mask):
        # feature_ho_bx = feature_ho_bx.long()
        all_mask_object = []
        for feature_ho_bx in feature_ho_bxs:
            mask_object = torch.ones((mask.shape[1], mask.shape[2]), dtype=bool).cuda()
            feature_ho_bx = feature_ho_bx.detach()
            feature_ho_bx = feature_ho_bx.long()
            for j in range(feature_ho_bx.shape[0]):
                w1, h1, w2, h2 = feature_ho_bx[j] // 32
                w1 = torch.minimum(w1, torch.tensor(mask.shape[2] - 1).cuda().long())
                w2 = torch.minimum(w2, torch.tensor(mask.shape[2] - 1).cuda().long())
                h1 = torch.minimum(h1, torch.tensor(mask.shape[1] - 1).cuda().long())
                ...
                if h2 + 1 < cur_mask_object.shape[0]:
                    cur_mask_object = cur_mask_object.index_fill(0,
                                                                 torch.arange(h2 + 1, cur_mask_object.shape[0]).cuda(),
                                                                 True)
                if w1 > 0:
                    cur_mask_object = cur_mask_object.index_fill(1, torch.arange(0, w1).cuda(), True)
                if w2 + 1 < cur_mask_object.shape[1]:
                    cur_mask_object = cur_mask_object.index_fill(1,
                                                                 torch.arange(w2 + 1, cur_mask_object.shape[1]).cuda(),
                                                                 True)
                mask_object = torch.minimum(mask_object, cur_mask_object)
            # mask_object = torch.stack(mask_object, 0)
            all_mask_object.append(mask_object)
        # mask_object = mask_object.view(mask.shape[0], self.num_queries, mask_object.shape[-2], mask_object.shape[-1])
        return all_mask_object

    def get_feature_human_mask(self, feature_ho_bxs, mask):
        # feature_ho_bx = feature_ho_bx.long()
        all_mask_object = []
        for feature_ho_bx in feature_ho_bxs:
            mask_object = torch.ones((mask.shape[1], mask.shape[2]), dtype=bool).cuda()
            ...
            feature_ho_bx = feature_ho_bx.long()
            for j in range(feature_ho_bx.shape[0]):
                w1, h1, w2, h2 = feature_ho_bx[j] // 40
                w1 = torch.minimum(w1, torch.tensor(mask.shape[2] - 1).cuda().long())
                w2 = torch.minimum(w2, torch.tensor(mask.shape[2] - 1).cuda().long())
                h1 = torch.minimum(h1, torch.tensor(mask.shape[1] - 1).cuda().long())
                h2 = torch.minimum(h2, torch.tensor(mask.shape[1] - 1).cuda().long())
    ...
    def forward(self,
        ...
        # 目标检测
        with torch.no_grad():
            results, hs, features, contact_masks = self.od_forward(self.detector, images, contact_masks)
        ...
        human_boxes = [r['human_boxes'] for r in region_props]
        object_boxes = [r['object_boxes'] for r in region_props]
        ...
        human_masks = torch.stack(self.get_feature_human_mask(human_boxes, mask), 0).reshape(-1, 1, h * w)
        object_masks = torch.stack(self.get_feature_ho_mask(object_boxes, mask), 0).reshape(-1, 1, h * w)
        c_masks = torch.where(contact_masks[2] != 0, False, True).reshape(-1, 1, h * w)

        query_embeds = []

        for i, (ho_q, mem, c_mk, h_mk, o_mk) in enumerate(zip(ho_queries, memory, c_masks, human_masks, object_masks)):
            if torch.sum(c_masks[i] == False) > 0:
                add_mask=torch.minimum(o_mk, c_mk)
            else:
                add_mask=torch.minimum(h_mk, o_mk)

            query_embeds.append(self.decoder_mask(
                ho_q.unsqueeze(1),  # (n, 1, q_dim)
                mem.unsqueeze(1),  # (hw, 1, kv_dim)
                kv_padding_mask=add_mask,  # (1, hw)
                # kv_padding_mask=c_mk,  # (1, hw)
                q_pos=positional_embeds[i],  # centre: (n, 1, 2*kv_dim), box: (n, 1, 4*kv_dim)
                k_pos=k_pos[i]  # (hw, 1, kv_dim)
            ).squeeze(dim=2) * self.contactalpha
                                + self.decoder(
                ho_q.unsqueeze(1),  # (n, 1, q_dim)
                mem.unsqueeze(1),  # (hw, 1, kv_dim)
...
def build_detector(args, obj_to_verb):
    ...
    triplet_decoder_mask = TransformerDecoder(
        decoder_layer=decoder_layer,
        num_layers=1
    )

    # new
    # contact_decoder = TransformerDecoder(
    #     decoder_layer=decoder_layer,
    ...
    model = CAICL(
        ...
        triplet_decoder_mask=triplet_decoder_mask,
        ...
        contactalpha=args.contactalpha,
    )
    return model
