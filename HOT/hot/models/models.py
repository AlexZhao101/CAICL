    ...
    def forward(self, feed_dict, *, segSize=None):
        ...
        if segSize is None:
            if self.deep_sup_scale is not None: # use deep supervision technique
                ...
                # x,x_part
                pred, pred_p, pred_feat = self.decoder(self.encoder(feed_dict['img_data'], return_feature_maps=True))

            loss = self.crit(pred, feed_dict['seg_label'])
            if pred_p is not None:
                loss = loss + 0.1*self.crit(pred_p, feed_dict['part_label'])
            ...
            # acc = self.pixel_acc(pred_p, feed_dict['part_label'])
            return loss, acc
        # inference
        else:
            pred, pred_b = self.decoder(self.encoder(feed_dict['img_data'], return_feature_maps=True), segSize=segSize)
            return pred, pred_b
...
class PartSegm(nn.Module):
    ...
    def __init__(self, nChannels, num_class=1, use_conv=False, use_contrastive=False):
        ...
        self.use_contrastive = use_contrastive
        self.sm_dim = 'channel'
        self.cbr_part = conv3x3_bn_relu(nChannels, nChannels, 1)
        # kernel size=1, stride=1
        self.conv_last_part = nn.Conv2d(nChannels, num_class, 1, 1, 0)
        self.combine_res = False
        if self.combine_res:
            cbr = [conv3x3_bn_relu(nChannels, nChannels, 1) for _ in range(num_class)]
            self.cbr = nn.ModuleList(cbr)
            #  self.cbr = conv3x3_bn_relu(nChannels, nChannels, 1)
            conv_last = [nn.Conv2d(nChannels, 1, 1, 1, 0) for _ in range(num_class)]
            self.conv_last = nn.ModuleList(conv_last)
        else:
            cbr = [conv3x3_bn_relu(nChannels, nChannels//8, 1) for _ in range(num_class)]
            self.cbr = nn.ModuleList(cbr)
            self.conv_last_cont = nn.Conv2d(nChannels//8*num_class, num_class, 1, 1, 0)
        if use_conv:
            self.conv1x1 = nn.Conv2d(nChannels, 1, kernel_size=1)

    def forward(self, x, x_part):
        # conv_bn_relu
        x_part = self.cbr_part(x_part)
        x_part = self.conv_last_part(x_part)
        # Batch-size,身体部位数+背景,height,weight
        B, C, H, W = x_part.shape
        if self.sm_dim == 'channel':
            attention = nn.functional.softmax(x_part, dim=1)
        else:
            attention = nn.functional.softmax(x_part.view(B, C, -1), dim=-1).view(B, C, H, W)
        attended_features = []
        keep_feautres = []
        for i in range(self.num_class):
            attended_feature = x * attention[:, i, :, :].unsqueeze(1).expand_as(x)
            attended_feature = self.cbr[i](attended_feature)
            if self.use_contrastive:
                keep_feautres.append(torch.sum(attended_feature, dim=(2,3)))
            if self.combine_res:
                attended_feature = self.conv_last[i](attended_feature)
            #  if self.use_conv:
                #  attended_feature = self.conv1x1(attended_feature)
            attended_features.append(attended_feature)

        if self.use_contrastive:
            x_feat = torch.stack(keep_feautres, dim=1)
        else:
            x_feat = None
        x = torch.cat(attended_features, dim=1)
        if not self.combine_res:
            x = self.conv_last_cont(x)
        #  print(x_feat.shape, x.shape)
        return x, x_part, x_feat
...
class C1(nn.Module):
    def __init__(self, num_class=150, fc_dim=2048, use_softmax=False, with_binary=False, with_part=False, use_contrastive=False):
        ...
        self.cbr = conv3x3_bn(fc_dim, fc_dim // 4, 1)
        self.cbr_part = conv3x3_bn(fc_dim, fc_dim // 4, 1)
    ...
    def forward(self, conv_out, segSize=None):
        ...
        if self.with_part:
            ...
            x, x_part, x_feat = self.part_branch(x, x_part)
        else:
            x = self.conv_last(x)
        ...
        if self.use_softmax: # is True during inference
            ...
            if self.with_part:
                ...
                return x, None
            # if self.with_binary:
            #     # _att = nn.functional.interpolate(
            #     #     _att, size=segSize, mode='bilinear', align_corners=False)
            #     x_b = nn.functional.interpolate(
            #           x_b, size=segSize, mode='bilinear', align_corners=False)
            #     x_b = nn.functional.softmax(x_b, dim=1)
        else:
            x = nn.functional.log_softmax(x, dim=1)
            if self.with_part:
                ...
                return x, None, None
            # x_b = nn.functional.log_softmax(x_b, dim=1)
