...
def precision(preds, label):
    ...
    #  valid = (label >= 0)
    acc_sum = (valid * (preds == label)).sum()
    valid_sum = pred_valid.sum()
    prec = float(acc_sum) / (valid_sum + 1e-10)
    return prec, valid_sum


def accuracy_binary(preds, label):
    valid = (label > 0)
    #  valid = (label >= 0)
    acc_sum = (valid * ((preds>0) == (label>0))).sum()
    valid_sum = valid.sum()
    acc = float(acc_sum) / (valid_sum + 1e-10)
    return acc, valid_sum

def accuracy_label(preds, label, part_label):
    valid = (label == part_label)
    #  valid = (label >= 0)
    acc_sum = (valid * ((preds==part_label) == (label==part_label))).sum()
    valid_sum = valid.sum()
    acc = float(acc_sum) / (valid_sum + 1e-10)
    return acc, valid_sum

def contact_acc(preds, label):
    preds = np.asarray(preds).astype(int).copy()
    label = np.asarray(label).astype(int).copy()
    uid = np.unique(label)
    ...
    for _id in uid:
        ...
        tmp_pred = (preds == _id)
        tmp_label = (label == _id)
        area_intersection = np.sum(tmp_pred * tmp_label)
        area_union = np.sum((tmp_pred + tmp_label) > 0)
        for idx, _iou in enumerate(ious):
            if area_intersection/area_union >= _iou:
                tp[idx] += 1

    return tp, contact_sum
...
def intersectionAndUnion(imPred, imLab, numClass):
    ...
    # Compute area union:
    (area_pred, _) = np.histogram(imPred, bins=numClass-1, range=(1, numClass-1))
    (area_lab, _) = np.histogram(imLab, bins=numClass-1, range=(1, numClass-1))
    area_union = area_pred + area_lab - area_intersection

    return (area_intersection, area_union)

def intersectionAndUnion_all(imPred, imLab, numClass):
    imPred = np.asarray(imPred).copy()
    imLab = np.asarray(imLab).copy()
