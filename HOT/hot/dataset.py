...
class TrainDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_per_gpu):
            ...
            part = imresize(part, (batch_widths[i], batch_heights[i]), interp='nearest')
            ...
            # further downsample seg label, need to avoid seg label misalignment
            part_rounded_width = self.round2nearest_multiple(part.size[0], self.segm_downsampling_rate)
            part_rounded_height = self.round2nearest_multiple(part.size[1], self.segm_downsampling_rate)
            part_rounded = Image.new('L', (part_rounded_width, part_rounded_height), 0)
            part_rounded.paste(part, (0, 0))
            part = imresize(
                part_rounded,
                (part_rounded.size[0] // self.segm_downsampling_rate,
                 part_rounded.size[1] // self.segm_downsampling_rate),
                interp='nearest')
        ...
        # output['seg_binary'] = (batch_segms > 0).long()
        return output
